...
env_init(void)
{
	...
	// Assign NULL to the last env_link, so that env_alloc()
	// can handle the E_NO_FREE_ENV situation properly.
	envs[NENV - 1].env_id = 0;
	envs[NENV - 1].env_status = ENV_FREE;
	envs[NENV - 1].env_link = NULL;

	// envs[0] is the first free env.
	env_free_list = envs;
...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
	...
	// LAB 3: Your code here.
	// Give each env its own stack of size 2 * PGSIZE
...
bind_functions(struct Env *e, struct Elf *elf)
{
	...
	// and e_shnum holds the number of entries in the section header
	// table.
	...
	esymtab = NULL;
	strtab = NULL;
	// As per the ELF specification, an object file may have only one
	// SYMTAB section (though this restriction may be relaxed in the 
	// future), and it may have multiple STRTAB sections. The tables 
	// we are looking for are called '.symtab' and '.strtab'.
	...
	// Look up the symbol and symbol string tables.
	for (; sh < esh; sh++)
	{
		if (sh->sh_type == ELF_SHT_SYMTAB /* &&
			!strcmp(".symtab", sect + sh->sh_name) */)
		{
			// Store the SYMTAB bounds.
			symtab = (struct Elf32_Sym *) ((uint8_t *) elf + 
				sh->sh_offset);
			esymtab = (struct Elf32_Sym *) ((uint8_t *) symtab +
				sh->sh_size);
		}
		else if (sh->sh_type == ELF_SHT_STRTAB && 
				!strcmp(".strtab", sect + sh->sh_name))
		{
			// Store the STRTAB beginning.
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	if ((elfh = (struct Elf *) binary)->e_magic != ELF_MAGIC)
	{
	...
	// Store the program's entry point in the Trapframe's EIP,
	// so that env_pop_tf() can load it into the actual EIP for
	// the environment to start executing there.
	e->env_tf.tf_eip = elfh->e_entry;
	
#ifdef CONFIG_KSPACE
	// Uncomment this for task №5.
	bind_functions(NULL, elfh);
#endif
...
env_create(uint8_t *binary, size_t size, enum EnvType type)
{
	...
	// Allocation failure.
	if ((err_code = env_alloc(&new_env, 0)))
	{
		panic("env_create: env_alloc: %i", err_code);
	}
